...
DECLARE
  ...
  v_links jsonb;
BEGIN
  SELECT id, fixed_name, slug, avatar_url INTO v_aff FROM public.affiliates WHERE lower(slug) = lower(_affiliate_slug) LIMIT 1;
  ...
  RETURN jsonb_build_object(
    'affiliate', jsonb_build_object('id', v_aff.id, 'name', v_aff.fixed_name, 'slug', v_aff.slug, 'avatar_url', v_aff.avatar_url),
